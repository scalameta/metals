...
class WorkspaceEditWorksheetPublisher() extends WorksheetPublisher {
  ...
  private def render(path: AbsolutePath)(worksheet: EvaluatedWorksheet): WorkspaceEdit = {
  ...
  private def publish(languageClient: MetalsLanguageClient)(edit: WorkspaceEdit): Unit = {
  ...
  private def locatePreviousEdit(statement: EvaluatedWorksheetStatement, source: Input): Option[Position] = {
      ...
      editPattern.findFirstMatchIn(text).map (m => {
