...
abstract class Decorations[T](
  ...
  protected def syntheticDecorations(
      path: AbsolutePath,
      includeRange: Option[l.Range] = None,
  ): Seq[T] = {
    ...
    currentDocument(path) match {
      case Some(doc) if isFocusedDocument =>
        decorations(path, doc, includeRange)
      case _ =>
        Nil
    }
  }
  ...
  protected def decorations(
      ...
      inludeRange: Option[l.Range] = None,
  ): Seq[T] = {
    if (areSyntheticsEnabled) {
  ...
  private def typeDecorations(
    ...
    val typeDecorations = for {
      ...
      // TODO: s.Range has method encoses(l.Range), so we can simply check if tree range encloses param range
      // lspRange = realPosition.toLsp
      // if inludeRange.forall(_.encloses(lspRange))
      // signature <- textDocument.symbols.find(_.symbol == occ.symbol)
    } yield {
