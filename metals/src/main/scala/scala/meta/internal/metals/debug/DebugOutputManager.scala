...
class DebugOutputManager(
  ...
  private val activeSessionOutputs = new ConcurrentHashMap[String, SessionOutputBuffer]()
  
  // Cache for recently accessed historical sessions
  private val historicalSessionCache = new ConcurrentHashMap[String, List[OutputEventArguments]]()
  
  private val debugOutputDir = workspace.resolve(".metals").resolve("debug-output")
  ...
  def getOrCreateBuffer(sessionId: String): SessionOutputBuffer = {
    activeSessionOutputs.computeIfAbsent(sessionId, _ => 
      new SessionOutputBuffer(sessionId, maxInMemorySize, persistToDisk, debugOutputDir)
    )
  }
   ...
   */
  private def getHistoricalOutputs(sessionId: String): Seq[OutputEventArguments] = {
    ...
    Option(historicalSessionCache.get(sessionId)).getOrElse {
      ...
      if (Files.exists(outputFile)) {
        val outputs = Files.readAllLines(outputFile, StandardCharsets.UTF_8).asScala.map { line =>
  ...
  def listSessions(): List[String] = {
    ...
    val historicalSessions = if (persistToDisk && Files.exists(debugOutputDir.toNIO)) {
      Files.list(debugOutputDir.toNIO).iterator().asScala
