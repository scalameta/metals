...
object BazelMavenJsonImporter {
  ...
  def sanitizeCoordinate(group: String, artifact: String): String = {
   ...
   */
  def hubCoordinateVersions(hub: MavenHub): Map[String, String] = {
    val parsed = Try {
      ...
      Option(json.getAsJsonObject("artifacts")) match {
        ...
        case None =>
          val dependencies = Option(json.getAsJsonObject("dependency_tree"))
            .flatMap(dt => Option(dt.getAsJsonArray("dependencies")))
          dependencies.toList.flatMap(coordinateVersionsFromLegacyFormat)
      }
    }.getOrElse(Nil)
    parsed.toMap
  }
  ...
  def labelCoordinate(label: String): Option[String] =
   ...
   */
  def labelHubName(label: String): Option[HubName] = {
          ...
          .orElse {
            ...
            scribe.warn(
              s"bazel-mbt: ambiguous external dep '$label' (coordinate '$coord') " +
                s"matched ${candidates.size} module versions " +
                s"[${candidates.map(_.version).mkString(", ")}]; " +
                s"no per-hub version match, picking '${fallback.version}'"
            )
            Some(fallback)
          }
        chosen.map(_.id)
