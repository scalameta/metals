...
object ScalaToolchainModules {
  ...
  case class Resolution(
     ...
     * compiler bundle transitively contains the library). Suppression is
     * version-agnostic.
     */
    def moduleIdsFor(
  ...
  private case class TestingToolchain(
      repoMarker: String,
      organization: String,
  )

  private val testingToolchains: List[TestingToolchain] = List(
    TestingToolchain("io_bazel_rules_scala_scalatest", "org.scalatest"),
    TestingToolchain("io_bazel_rules_scala_scalactic", "org.scalactic"),
  )
  ...
  def testingModules(
    ...
    val modules = for {
      (version, dir, toolchain) <- testingToolchainDirs(externalDir)
      (jar, artifact, artifactVersion) <- listDirectory(dir)
        .sortBy(_.getFileName.toString)
        .flatMap { candidate =>
          candidate.getFileName.toString match {
            case mavenStyleJarName(artifact, artifactVersion) =>
              Some((candidate, artifact, artifactVersion))
            case _ => None
          }
        }
        .headOption
      ...
      version -> MbtDependencyModule(
        id = s"${toolchain.organization}:$artifact:$artifactVersion",
   ...
   * uses the testing toolchain but Bazel hasn't fetched the jars yet" apart
   * from "this workspace doesn't use it at all" (expected to yield nothing).
   */
  def testingToolchainVersions(externalDir: Path): Set[String] =
    testingToolchainDirs(externalDir).map { case (version, _, _) =>
      version
    }.toSet

  private def testingToolchainDirs(
      externalDir: Path
  ): List[(String, Path, TestingToolchain)] =
    for {
      dir <- listDirectory(externalDir)
      if Files.isDirectory(dir)
      name = dir.getFileName.toString
      toolchain <- testingToolchains.find(t => name.contains(t.repoMarker))
      version <- trailingScalaVersion
        .findFirstMatchIn(name)
        .map(_.group(1).replace('_', '.'))
    } yield (version, dir, toolchain)
  ...
  private def fetchBundle(
    ...
    Fetch()
      ...
      .map { result =>
        val byModule = result.fullDetailedArtifacts0
          .collect { case (dep, Right(publication), _, Some(file)) =>
            (
              dep.module,
      ...
      .recover { case NonFatal(error) =>
        scribe.warn(
          s"bazel-mbt: could not resolve Scala toolchain jars for " +
            s"${dependency.module.repr}:${dependency.versionConstraint.asString}: $error"
        )
        Nil
      }
