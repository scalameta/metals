...
class ExtractMethodCodeAction(
  ...
  override def contribute(params: CodeActionParams, token: CancelToken)(implicit
    ...
    val asgnOpt = trees.findLastEnclosingAt[Defn.Val](
      path,
      range.getStart(),
      asgn => asgn.rhs.pos.encloses(range)
    )
    pprint.pprintln("Jestem w contribute2")
    pprint.pprintln(asgnOpt)
    val textEdits = for {
      asgn <- asgnOpt
      parent <- asgn.parent
      stats <- lastEnclosingStatsList(parent)
      _ = pprint.pprintln(stats)
      argument = asgn.rhs
      // avoid extracting lambdas (this needs actual type information)
      if isNotLambda(argument)
      stat <- stats.find(stat => stat.pos.encloses(asgn.pos))
      ...
      val args = "Tu beda argumenty"
      val valueText = s"${indent}def $name ($args) = { ${argument.toString()} }"
      val valueTextWithBraces = withBraces(stat, source, valueText, blank)
      // we need to add additional () in case of  `apply{}`
      val replacementText = s"name($args)"
    ...
    textEdits match {
      case Some(edits) =>
        ...
        codeAction.setTitle(ExtractMethodCodeAction.title)
