...
class SQLQueries/*SQLQueries.scala*/ {
    ...
    val nestedInterpolators/*SQLQueries.scala*/ = sql/*SQLQueries.scala*/"""
        SELECT name, age
        ${s/*StringContext.scala fallback to scala.StringContext#*/"FROM ${sql/*SQLQueries.scala*/"users"} WHERE"} age > 30.5
        """
}