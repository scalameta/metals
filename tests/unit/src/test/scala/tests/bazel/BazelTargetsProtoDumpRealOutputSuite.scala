...
/**
 * Pins [[BazelTargetsProtoDump]] against REAL `bazel query` output, complementing
 * the hand-built JSON in `BazelTargetsProtoDumpSuite` by exercising the exact wire
 * shape (field names, nesting, `SELECTOR_LIST` framing) Bazel actually emits.
 *
 * The fixture `bazel/rules-scala-fullinfo-fragments.jsonl` is a verbatim slice of
 * the importer's full-information query against the bazelbuild/rules_scala repo
 * (`v7.2.5-5-g156e65c`, Bazel 7.7.1) — the same query
 * [[scala.meta.internal.metals.mbt.importer.BazelMbtImporter]].extract feeds to
 * the dump:
 * {{{
 * bazel query --output=streamed_jsonproto --proto:flatten_selects=false \
 *   --proto:output_rule_attrs=srcs,scalacopts,javacopts,scala_version --keep_going \
 *   "deps(set(//third_party/dependency_analyzer/... //src/java/io/bazel/rulesscala/scalac/reporter/...))"
 * }}}
 * Targets were chosen to cover the most edge cases with no overlap; only per-rule
 * noise the dump never reads (`location`, `$rule_implementation_hash`) and a
 * transient `--keep_going` "unknown repo" artifact were stripped, and the
 * machine-specific `location` of the non-rule targets.
 */
class BazelTargetsProtoDumpRealOutputSuite extends BaseSuite {
  ...
  private val dump: BazelTargetsProtoDump = {
    val path = AbsolutePath(BuildInfo.testResourceDirectory)
      .resolve("bazel")
      .resolve("rules-scala-fullinfo-fragments.jsonl")
    new BazelTargetsProtoDump(Files.readString(path.toNIO))
  }
  ...
  test("srcs-select-groups-sources-by-scala-version") {
    ...
    assertEquals(dump.getLabels("srcs")(scalaVersion), List(v2, v3))
  }
  ...
  test("filegroup-sources-are-inlined-and-deduplicated") {
    ...
    assertEquals(dump.getLabels("srcs")(scalac), expanded)
    // Querying the filegroup itself yields the same deduplicated set.
    assertEquals(dump.getLabels("srcs")(scalacFiles), expanded)
  ...
  test("plain-label-list-srcs-may-be-an-external-label") {
    ...
    assertEquals(
      dump.getLabels("srcs")(depReportingCompiler),
  ...
  test("deps-are-direct-but-external-deps-are-transitive-and-external-only") {
    ...
    val external = dump.externalDepsByTarget(List(scalac))(scalac)
