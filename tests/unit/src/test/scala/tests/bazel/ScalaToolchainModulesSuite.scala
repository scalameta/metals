...
class ScalaToolchainModulesSuite extends BaseSuite {
  ...
  test("existing-maven-module-suppresses-same-artifact") {
    // Suppression is by organization:name, version-agnostic (maven 2.12.18
    // suppresses toolchain 2.12.21).
    assertEquals(
      toolchain.moduleIdsFor(
        Some("2.12.21"),
        List("//pkg/b:lib"),
  ...
  test("testing-modules-discovered-from-bazel-external-dir") {
    ...
    repo(
      ...
      // A decoy jar that passes the `.jar`/not-`-src.jar` filter but is not
      // maven-style (no `-<version>`); it sorts before the real artifact, so the
      // old "first non-src jar then match" logic yielded nothing for this repo.
      "scalatest.jar",
  ...
  test("maven-repos-embedding-scalatest-are-not-testing-toolchain-repos") {
    val external = java.nio.file.Files.createTempDirectory("external")
    def repo(name: String, jars: String*): Unit = {
      val dir = external.resolve(name)
      java.nio.file.Files.createDirectories(dir)
    ...
    // A maven artifact repo whose canonical name embeds "scalatest" and ends
    // in the artifact version: matching it would parse `3_2_19` into a bogus
    // Scala version key.
    repo("+maven+org_scalatest_scalatest_3_2_19", "scalatest_2.13-3.2.19.jar")
    repo(
      "+scala_deps+io_bazel_rules_scala_scalatest_2_13_16",
      "scalatest_2.13-3.2.19.jar",
    )
  ...
  test("from-discovery-srcjar-only-namespace-gets-toolchain-modules") {
    // A scala_library whose only source is a `.srcjar` (generated code, later
    // materialized into `.scala` files by BazelSrcjarSources) must still resolve
    // its Scala stdlib — the toolchain gate treats `.srcjar` as Scala-bearing.
    val build = MbtBuildFixture.fromDiscovery(
      granularity = BazelMbtNamespaceMode.BuildFile,
      targetLabels = List("//pkg/g:lib"),
      srcsByTarget = Map("//pkg/g:lib" -> List("//pkg/g:generated.srcjar")),
      scalacOptionsByTarget = Map.empty,
      javacOptionsByTarget = Map.empty,
      directDepRules = Map.empty,
      externalDepsByTarget = Map.empty,
      ...
      classDirectoriesByTarget = Map.empty,
      dependencyModules = Nil,
      scalaVersionByTarget = Map("//pkg/g:lib" -> Some("2.12.21")),
    ...
    val namespaces = build.getNamespaces.asScala
    assertEquals(
      namespaces("//pkg/g").getDependencyModuleIds.asScala.toSet,
      Set(library2),
    )
  }
  ...
  test(
    "from-discovery-java-compiler-classpath-namespace-gets-compiler-bundle"
  ) {
    // A java_library depending on scala_compile_classpath (e.g. rules_scala's
    // Reporter subclass) still needs the full compiler bundle.
    val build = MbtBuildFixture.fromDiscovery(
      granularity = BazelMbtNamespaceMode.BuildFile,
      targetLabels = List("//pkg/a:plugin"),
      srcsByTarget = Map("//pkg/a:plugin" -> List("//pkg/a:Reporter.java")),
      scalacOptionsByTarget = Map.empty,
      javacOptionsByTarget = Map.empty,
      directDepRules = Map.empty,
      externalDepsByTarget = Map.empty,
      ...
      inactiveSources = Map.empty,
      versionSpecificSourceLabels = Set.empty,
      // `//pkg/a:plugin` is a compiler-classpath target in `toolchain`.
      toolchain = toolchain,
    )
    val namespaces = build.getNamespaces.asScala
    assertEquals(
