...
import org.eclipse.lsp4j.{Location, Position, Range, TextDocumentPositionParams}
...
//  }
  ...
  test("definition-overloaded-apply") {
    for {
      _ <- initialize(
        """|
           |/metals.json
      ...
      _ = assertNoDiff(
        ...
        """|/a/src/main/scala/a/Main.scala
           |package a
           |object Main/*L1*/ {
           |  List/*package.scala*/(1).map/*List.scala*/(Foo/*Foo.scala:4*/.apply/*Foo.scala:5*/)
           |}
           |""".stripMargin,
      )
      ...
      mainPath = server.toPath("a/src/main/scala/a/Main.scala")
      fooPath = server.toPath("a/src/main/scala/a/Foo.scala")
      mousePositionAtApply = new Position(2, 19)
      objectFooApplyRange = new Range(new Position(5, 6), new Position(5, 11))
