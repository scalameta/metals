 ...
 *
 * Fixtures mirror the real rules_scala case behind the navigation bug: the
 * active `DepsTrackingReporter.java` copy (Scala 2.12) lives in the REAL
 * `reporter` namespace, while the `scala_3/` copy lives in an unreachable
 * `…@3.8.4` version-branch namespace.
 */
class MbtCrossVersionDefinitionSuite extends BaseSuite {
  ...
  private def best(
    ...
    candidates.minBy(_.rankKey(preferred))
