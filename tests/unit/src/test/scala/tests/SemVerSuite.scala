...
class SemVerSuite extends FunSuite {
  ...
  val expected: List[(String, SemVer.Version)] = List(
    ...
    ("3.7-M1", SemVer.Version(3, 7, 0, isStable = false)),
    ("3.7-4972921", SemVer.Version(3, 7, 4972921, isStable = false)),
    ("3.2-148-d9af944", SemVer.Version(3, 2, 148, isStable = false)),
    (
      "1.4.13-18-3cc4983b-20220225-1641",
      SemVer.Version(1, 4, 13, isStable = false),
    ),
    (
      "1.0.0-MF",
  ...
  test("fromString") {
    val incorrect = expected
      .map { case (s, e) => (s, e, SemVer.Version.fromString(s)) }
      .filter({ case (_, e, parsed) => parsed != e })
