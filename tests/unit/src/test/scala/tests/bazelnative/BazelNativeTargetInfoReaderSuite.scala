...
class BazelNativeTargetInfoReaderSuite extends FunSuite {
  ...
  test("parses JvmTargetInfo.transitive_source_jars") {
    val json = """{
      "id":"//src:lib",
      "kind":"scala_library",
      "jvm_target_info":{
        "transitive_source_jars":[
          {"path":"out/dep1-sources.jar","short_path":"dep1-sources.jar","is_source":false},
          {"path":"out/dep2-sources.jar","short_path":"dep2-sources.jar","is_source":false}
        ]
      }
    }"""
    val info = BazelNativeTargetInfoReader.parseJson(json)
    val jars = info.jvmTargetInfo.get.transitiveSourceJars
    assertEquals(jars.size, 2)
    assertEquals(
      jars.map(_.path),
      List("out/dep1-sources.jar", "out/dep2-sources.jar"),
    )
  }

  test("parses JvmTargetInfo.transitive_runtime_jars") {
    val json = """{
      "id":"//src:lib",
      "kind":"scala_library",
      "jvm_target_info":{
    ...
    val info = BazelNativeTargetInfoReader.parseJson(json)
    val jars = info.jvmTargetInfo.get.transitiveRuntimeJars
    assertEquals(jars.size, 2)
    assertEquals(jars.map(_.path), List("out/dep1.jar", "out/dep2.jar"))
  }
