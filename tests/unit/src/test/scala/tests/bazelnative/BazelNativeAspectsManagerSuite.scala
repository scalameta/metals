...
class BazelNativeAspectsManagerSuite extends FunSuite {
  ...
  test("aspect .bzl file is written to .metals-bsp/aspects/") {
  ...
  test("BUILD file is written alongside .bzl") {
    withTempWorkspace { workspace =>
      ...
      val buildFile =
        workspace.resolve(".metals-bsp").resolve("aspects").resolve("BUILD")
  ...
  test("--aspects flag points to correct path") {
    withTempWorkspace { workspace =>
      ...
      assertEquals(
        manager.aspectsFlag,
        "//.metals-bsp/aspects:bsp_target_info.bzl%bsp_target_info_aspect",
      )
    }
  }
