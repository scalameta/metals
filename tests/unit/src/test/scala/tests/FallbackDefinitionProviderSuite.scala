...
/**
 * Pins [[FallbackDefinitionProvider]]'s `index.definitions(...).filter(_.path
 * .exists)` behaviour: when the index still knows a definition whose source
 * file has since been deleted, the fallback must skip it in favour of one whose
 * file still exists (and must return nothing when every candidate's file is
 * gone) rather than pointing navigation at a missing file.
 */
class FallbackDefinitionProviderSuite extends BaseSuite {

  // A `GlobalSymbolIndex` that, for any queried symbol, reports it as defined in
  // each of `paths` (echoing the symbol so the provider's `definitionSymbol`
  // filter keeps every entry). `paths` order is preserved, so a non-existent
  // path listed first would win without the `path.exists` filter.
  ...
  private def search(
    ...
    val provider = new FallbackDefinitionProvider(trees, index)
  ...
  test("prefers-existing-file-over-deleted-definition") {
    ...
    // Deleted listed first: without the `path.exists` filter it would win.
