...
object CompletionDocSuite extends BaseCompletionSuite {
  ...
  check(
    ...
    """|> A container object which may or may not contain a `int` value.
       ...
       |value are provided, such as `#orElse(int)`
       |(return a default value if value not present) and
       |`#ifPresent(java.util.function.IntConsumer)` (execute a block
  ...
  check(
    ...
    """|DeprecatedPredef scala
       ...
       |
       |###  Console Output
       | For basic console output, `Predef` provides convenience methods print(x:Any* and println(x:Any*,
