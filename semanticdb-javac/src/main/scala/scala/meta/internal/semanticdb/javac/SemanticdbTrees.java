...
public class SemanticdbTrees {
	public SemanticdbTrees(GlobalSymbolsCache globals, LocalSymbolsCache locals, String semanticdbUri, Types types,
			Trees trees, HashMap<Tree, TreePath> nodes) {
	...
	private Semanticdb.Tree annotationParameter(ExpressionTree expr) {
		if (expr instanceof MemberSelectTree) {
			...
			return tree(selectTree(tree(idTree(globals.semanticdbSymbol(expressionSym.getEnclosingElement(), locals))),
			...
			return tree(applyTree(tree(idTree(ARRAY_SYMBOL)),
					rhs.getInitializers().stream().map(this::annotationParameter).collect(Collectors.toList())));
			...
			if (value == null)
				...
				constant = stringConst((String) value);
			else if (value instanceof Boolean)
				constant = booleanConst((Boolean) value);
			else if (value instanceof Byte)
				constant = byteConst((Byte) value);
			else if (value instanceof Short)
				constant = shortConst((Short) value);
			else if (value instanceof Integer)
				constant = intConst((Integer) value);
			else if (value instanceof Long)
				constant = longConst((Long) value);
			else if (value instanceof Character)
				constant = charConst((Character) value);
			else if (value instanceof Float)
				constant = floatConst((Float) value);
			else if (value instanceof Double)
				constant = doubleConst((Double) value);
			else
				throw new IllegalStateException(semanticdbUri
						+ ": annotation parameter rhs was of unexpected class type " + value.getClass() + "\n" + value);
			...
			return tree(binopTree(annotationParameter(binExpr.getLeftOperand()),
					semanticdbBinaryOperator(expr.getKind()), annotationParameter(binExpr.getRightOperand())));
		} else if (expr instanceof UnaryTree) {
			UnaryTree unaryExpr = (UnaryTree) expr;
			return tree(unaryOpTree(semanticdbUnaryOperator(unaryExpr.getKind()),
			...
			return tree(castTree(typeVisitor.semanticdbType(getTreeType(tree.getType())),
					annotationParameter(tree.getExpression())));
		} else {
			throw new IllegalArgumentException(semanticdbUri
					+ ": annotation parameter rhs was of unexpected tree node type " + expr.getClass() + "\n" + expr);
		}
	}
	...
	private Semanticdb.BinaryOperator semanticdbBinaryOperator(Tree.Kind kind) {
		...
		default:
			throw new IllegalStateException(semanticdbUri + ": unexpected binary expression operator kind " + kind);
		}
	}
	...
	private Semanticdb.UnaryOperator semanticdbUnaryOperator(Tree.Kind kind) {
		...
		default:
			throw new IllegalStateException(semanticdbUri + ": unexpected unary expression operator kind " + kind);
		}
	}
}
