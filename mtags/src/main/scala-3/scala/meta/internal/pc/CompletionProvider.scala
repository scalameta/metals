...
class CompletionProvider(
  ...
  extension (l: List[Completion]) {
    def filterInteresting: List[Completion] =
      ...
      def isInteresting(head: Completion): Boolean =
        ...
        isNotLocalForwardReference then
          isSeen += id
          buf += head
          true
        else false
  ...
  private def computeRelevancePenalty(
    ...
    if !sym.isLocalToBlock then relevance |= IsNotLocalByBlock
    ...
    if sym.isField then relevance |= IsNotGetter
    ...
    if sym.is(Implicit) then relevance |= IsImplicitConversion
    ...
    if sym.is(Synthetic) then relevance |= IsSynthetic
          ...
          if byFuzzy != 0 then byFuzzy
            ...
            if byIdentifier != 0 then byIdentifier
              ...
              if byOwner != 0 then byOwner
                ...
                else s1.show.compareTo(s2.show)
