...
class PcConvertToEnum(
  ...
  def convertToEnum: ju.List[TextEdit] =
    ...
    Interactive.pathTo(driver.openedTrees(uri), pos) match
      case (t @ TypeDef(name, rhs: Template[_])) :: tail if t.symbol.exists && t.symbol.is(Flags.Sealed) =>
  ...
  private def collectImplementations(sym: ClassSymbol)(using Context) = {
    val collector = new DeepFolder[(List[TypeDef[_]], Option[TypeDef[_]])]({
        case ((acc, companion), t @ TypeDef(_, _: Template[_])) if t.symbol.isClass && !t.symbol.is(Flags.Synthetic) && t.symbol.info.parents.map(_.typeSymbol).exists(_ == sym) => 
          (t :: acc, companion)
        case ((acc, None), t @TypeDef(_, _: Template[_])) if t.symbol.isClass && t.symbol.companionClass == sym =>
          (acc, Some(t))
        case (acc, _) => acc
      })
  ...
  private def toDelete(sealedClassSymbol: ClassSymbol, implementations: List[TypeDef[_]], companionTree: Option[TypeDef[_]])(using Context) =
    val moduleClass = sealedClassSymbol.companionClass
    val deleteCompanionObject = companionTree.collect:
      case ct @ TypeDef(name, t: Template[_]) if getRelevantBodyParts(t).forall(implementations.contains(_)) =>
        expandedPosition(ct)
    val toReexport =
  ...
  private def makeReplaceText(name: TypeName, rhs: Template[_], implementations: List[TypeDef[_]], baseIndent: String, useFewerBraces: Boolean): String =
    val constrString = showFromSource(rhs.constr)
    val (simpleCases, complexCases) =
      if constrString == ""
      then implementations.partition(_.rhs.asInstanceOf[Template[_]].constr.paramss == List(Nil))
  ...
  private def getRelevantBodyParts(rhs: Template[_]): List[Tree] =
    def isParamOrTypeParam(stat: Tree): Boolean = stat match
      case stat: ValDef[_] => stat.symbol.is(Flags.ParamAccessor)
      case stat: TypeDef[_] => stat.symbol.is(Flags.Param)
      case _ => false
  ...
  private def detectIndentation(tree: TypeDef[_]): Int =
  ...
  private def tdefName(tdef: TypeDef[_]) = 
