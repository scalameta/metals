...
public class Metric {
  ...
  /**
   * A counter is an ever increasing value and each metric recorded increases the * counter. An
   * example are the number of API calls.
   *
   * <p>A gauge is a metric is has a certain value and each metric recorded records a new value. An
   * example are the number of API rate limited calls left in an hour.
   *
   * <p>Similar to a histogram, a summary samples observations (usually things like request
   * durations and response sizes). While it also provides a total count of observations and a sum
   * of all observed values, it calculates configurable quantiles over a sliding time window.
   *
   * <p>A histogram is a more limited version of metric where instead of recording the exact value,
