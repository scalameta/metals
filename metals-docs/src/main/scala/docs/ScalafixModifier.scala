...
class ScalafixModifier extends StringModifier {
  ...
  override def process(
    ...
    s"""|## Running scalafix rules
        ...
        |This should run all the rules defined in `.scalafix.conf` file. All built-in rules
        ...
        |follows the same convention as the [coursier dependencies](https://get-coursier.io/).
        |
