...
class CoursierComplete(scalaVersion: String) {
  ...
  def completeVersions(
    ...
    val (stable, preRelease) = completions(adjusted)
      ...
      .sortWith { case ((leftLabel, leftVersion), (rightLabel, rightVersion)) =>
        val versionComparison = Version.stableFirst.compare(
          leftVersion,
          rightVersion
        )
        versionComparison < 0 || (versionComparison == 0 && leftLabel > rightLabel)
      }
