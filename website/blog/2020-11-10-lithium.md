...
possible in Visual Studio Code. In other editors the additional information is
available via hover and new `With synthetics added` section, which shows how the
whole current line would look with the additional decorations.
...
[bkeepers/dotenv](https://github.com/bkeepers/dotenv). Example
json configurations above are defined for Visual Studio Code, the `type` and
`name` fields can be omitted when using in other clients.
